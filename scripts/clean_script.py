# scripts/clean_script_v13.py
import json
import os
import re

INPUT_FILE = "data/raw/wiki_pages.json"
OUTPUT_FILE = "data/kamus_terstruktur.json"
...
def extract_contoh_belgong(text):
    ...
    # Bersihkan dari bullet points
    contoh_text = re.sub(r'[•\-*]\s*', '', contoh_text)
    contoh_text = clean_text(contoh_text)
    
    # Ambil hanya kalimat pertama (yang benar-benar contoh)
    sentences = re.split(r'(?<=[.!?])\s+', contoh_text)
    # Filter: hanya ambil kalimat yang mengandung kata "disebut" atau "perhiasan" atau "manik"
    filtered = []
    for s in sentences:
        if any(kw in s.lower() for kw in ['disebut', 'perhiasan', 'manik', 'kalung']):
            filtered.append(s)
    return filtered[:1] if filtered else []  # Ambil 1 contoh
...
def extract_contoh_alas(text):
    ...
    # Pisahkan dengan pola yang lebih akurat
    examples = []
    # Pola 1: "Alas xxx : ..." atau "Alós xxx : ..."
    patterns = [
        r'(Alas\s+[^:]+:[^.!?]+[.!?])',
        r'(Alós\s+[^:]+:[^.!?]+[.!?])',
        r'(Nge ara[^.!?]+[.!?])',
    ...
    # Jika tidak ada, ambil kalimat yang mengandung kata kunci
    if not examples:
    ...
    return examples[:8] if examples else []  # Batasi 8 contoh
...
def main():
    print("🔄 Memulai pembersihan data (v13)...")
    ...
    for page in raw_pages:
        ...
        if title.lower() == "home" or not any(kw in text for kw in ["Salabisasi", "Kelas"]):
            ...
            print(f"📝 '{title}' dilewati (bukan entri kamus).")
            continue
        
        entry = parse_wiki_entry(text, title)
        if entry and (entry.get('makna') or entry.get('kelas_kata')):
            kamus.append(entry)
            print(f"✅ '{title}' berhasil diekstrak.")
        else:
            skipped.append(title)
            print(f"⚠️  '{title}' tidak memiliki data cukup.")
    
    os.makedirs(os.path.dirname(OUTPUT_FILE), exist_ok=True)
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
        json.dump(kamus, f, indent=2, ensure_ascii=False)
    
    with open("data/skipped_pages.json", 'w', encoding='utf-8') as f:
    ...
    print(f"📊 Statistik:")
    print(f"   - Total halaman: {stats['total']}")
